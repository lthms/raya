...
To join a k3s cluster, an agent needs to prove it knows a secret token to the
control plane. This token is usually generated by the contol plane on its first
boot, alongside its certificate authorities.

This approach is cumbersome, because it requires to instrument the control
plane to upload its token to a vault of sorts and the agents to wait for the
secret to be available.

To avoid this friction, we take a different road. Both the token and the CA are
instead generated by Terraform (using `hashicorp/random` and `hashicorp/tls`
respectively) and provisioned to the control plane via Ignition.

Agents receive a token in `k3s`’ full form, that is

```
K10<sha256 of the server CA certificate>::server:<secret>
```

This enables mutual authentication of the control plane and the agents, as they
...
[^notexactly]: In reality, this is a little more complicated. The Ignition
    ...
    will use to join the `k3s` cluster, so claiming we are using the
    `user_data` to decide the name introduces a circular dependency. In
    reality, we run the template a first time, using mock data for some key
    fields, to get a sha256 digest we can use to run the template a second
    time.

This naming scheme allows the cluster to detect when a VM has been replaced.
Seeing a node `agent-0-ca817dde` marked as `NotReady` and another node
`agent-0-896ef0a5` marked as `Ready` is enough for the cluster to know that the
former no longer exists. `raya` has a CronJob named `node-reaper` responsible
for detecting such a situation and cleaning up: deleting the node will cascade
into deleting zombie `VolumeAttachment` objects[^scaledown].

[^scaledown]: `node-repear` is also responsible for handling the removal of
    agents.
...
- `A` record for `cp.$google_dns_managed_zone.primary.dns_name` pointing to the
  control plane.
- `A` record for `a$i.$google_dns_managed_zone.primary.dns_name` pointing to
  the `i`th agents in the `agents.json` list.
